...
%central timeserie of null removal     
 ...
 \node[right=of ts_c0r] (scale1) {$\xLongrightarrow{\qquad\qquad\qquad} $};
 \node[below= of scale1] (scale2) {$x^\prime = \frac{x-\mu}{\sigma}$};
     
     
%central timeserie of scaled ones     
 \matrix[mmat, right=of scale1  , name path=ts_c0r] (ts_c0s) {-1.2 & -0.7 & +1.0 & +1.0 \\ };
 %timeseries below central one
 \matrix[mmat, below=of ts_c0s , name path=ts_b1s] (ts_b1s) {-1.3 & -0.1 & -0.1 & +1.5 \\};
 \matrix[mmat, below=of ts_b1s , name path=ts_b2s] (ts_b2s) {-0.2 & -0.2 & +1.6 & -1.2 \\};
 %timeseries above central one
 ...
 \matrix[mmat, above=of ts_a1s , name path=ts_a2s] (ts_a2s) {-0.6 & -0.6 & -0.6 & +1.7 \\ };
  ...
  (ts_c0s|-ts_full.south) node[below=0.5] {\textbf{Standard scaling}};
